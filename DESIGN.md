...
- Bright ceramic: `oklch(0.97 0.012 83)`
- Graphite: `oklch(0.205 0.006 83)`
- Titanium: `oklch(0.71 0.006 83)`
- Intention violet: `oklch(0.55 0.14 287)`
- Verified blue: `oklch(0.55 0.115 225)`
...
Mona Sans Variable is the display and reading family. Martian Mono is reserved for sparse chapter indices, progress, and proof labels. Headlines are short, compressed, and monumental; body copy is concise enough to read while the camera holds. Text remains semantic DOM content at native resolution. Gradient text and animated font-width distortion are prohibited.
...
GSAP ScrollTrigger owns normalized progress. Lenis may smooth eligible wheel input but cannot become a second clock. Camera and object motion are continuous, velocity-bounded, and exactly reversible. Long holds give each material event time to read. Copy enters only after the previous geometry has cleared its protected field. Reduced motion becomes a deliberate normal-flow document with a static authored frame.
